...
namespace KdTree
{
    public class PriorityQueue<TItem, TPriority> : IPriorityQueue<TItem, TPriority>
	{
		...
		public void Clear(int maxsize = 512)
        {
			// Reset the existing queue items
			for (int i = 0; i < count; i += 1)
            {
				queue[i].Item = default(TItem);
				queue[i].Priority = priorityMath.MinValue;
			}
			// Reset counter
			count = 0;
			// Optionally shrink queue
			if (capacity > maxsize)
            {
				capacity = maxsize;
				// Create a new queue
