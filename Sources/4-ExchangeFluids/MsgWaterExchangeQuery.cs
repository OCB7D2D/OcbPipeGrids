...
{
    public class MsgWaterExchangeQuery : ServerNodeQuery<NetPkgWaterExchangeQuery>
    {
        ...
        public override void ProcessOnWorker(NodeManagerWorker worker)
        {
            ...
            if (manager.TryGetNode(Position, out IExchangeFluids well))
            {
                response.Exchanged = well.ExchangeItem(HoldingCount, Factor);
                response.FillLevel = well.FillLevel;
            }
        ...
        protected override void SetupNetPkg(NetPkgWaterExchangeQuery pkg) => pkg.FromMsg(this);
